...
class MirrorModule:
	def __init__(self):
		...
		if firstSelected.find('Group__') == 0:
			...
			moduleNamespaceInfo = utils.stripAllNamespaces(firstSelected)
	...
	def mirrorModule_UI(self):
		...
		self.sameMirrorSettingForAll = False
		if len(self.modules) > 1:
			result = cmds.confirmDialog(title='Mirror Multiple Modules', message=str(len(self.modules)) + 'modules selected for mirror. \nHow would you like to apply mirror settings?', button=['Same for All','Individualy', 'Cancel'],defaultButton='Same for All',cancelButton='Cancel', dismissString='Cancel')
			
			if result == 'Cancel':
				return
				
			if result == 'Same for All':
				self.sameMirrorSettingForAll = True
		...
		if self.sameMirrorSettingForAll:
		...
		self.UIElements['buttonRow'] = cmds.rowLayout(nc=2, columnWidth=[(1,columnWidth),(1,columnWidth)],cat=[(1,'both',10),(2,'both',10)], columnAlign=[(1,'center'),(2,'center')])
	...
	def acceptWindow(self,*args):
		#a module info entry = (originalModule, mirroredModuleName, mirrorPlane, rotationFunction, translationFunction)
		self.moduleInfo = []
		...
		for i in range(len(self.modules)):
			...
			if utils.doesBlueprintUserSpecifiedNameExist(mirroredModuleUserSpecifiedName):
				cmds.confirmDialog(title='Name Conflic', message='Name \'' + mirroredModuleUserSpecifiedName + '\' already exists, aborting mirror',button=['Accept'], defaultButton='Accept')
				return
				
			rotationFunction = ''
			translationFunction = ''
			
			if self.sameMirrorSettingForAll == True:
	...
	def mirrorModules(self):
		mirrorModulesProgress_UI = cmds.progressWindow(title='Mirroring Modul(s)', status='This may take a few minutes...', isInterruptable=False)
