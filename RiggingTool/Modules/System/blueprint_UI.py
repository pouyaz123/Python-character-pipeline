...
class Blueprint_UI:
	...
	def initialiseModuleTab (self, tabHeight, tabWidth):
		...
		self.UIElements['moduleSpecific_Scroll'] = cmds.scrollLayout(hst=0,width=390)
	...
	def createModuleInstallButton(self, module):
		...
		cmds.scrollField(text=description,editable=False,w =self.scrollWidth - buttonSize - 16,h=buttonSize+8,wordWrap=True)
	...
	def initialiseTemplatesTab(self, tabHeight, tabWidth):
		...
		self.UIElements['templatesFrameLayout'] = cmds.frameLayout(height=(tabHeight-140), collapsable=False, borderVisible=False, labelVisible=False)
	...
	def createTemplateInstallButton(self, templateAndPath):
		...
		textColumn = cmds.columnLayout(columnAlign='center')
		cmds.text(align='center', width=self.scrollWidth-buttonSize-16, label=title)
		cmds.scrollField(text=description, editable=False,width=self.scrollWidth-buttonSize-16,h=buttonSize, wordWrap=True)
