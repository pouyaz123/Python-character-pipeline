...
TITLE = 'Leg IK Reverse Foot'

DESCRIPTION = 'This module provides an IK leg with reverse foot control.'
...
class LegIK_ReverseFoot(circleIK.CircleControlStretchyIK):
	...
	def install_custom(self,joints, moduleGrp,moduleContainer):
		...
		cmds.setAttr(footControl+'.rotateOrder',3)#3 = xzy
		...
		for joint in [ballJoint,toeJoint]:
			...
			ryControlInfo = controlObjectInstance.create(name,'yAxisCircle.ma',self, lod=2, translation=False, rotation=[False,True,False], globalScale=False,spaceSwitching=False)
